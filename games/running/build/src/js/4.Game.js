(function(MyGame, undefined) {
  'use strict';
    ...
    if(this.ctrl.isUp && this.pressed){
      this.elapsed = this.game.time.now - this.startPress;
      this.startPress = this.game.time.now;
      this.pressed = false;
      this.player.speed = limit(Math.min(this.elapsed,500), 500, 0, 0, 5);
      // this.player.sprite.animation.animations.walk;
      this.player.sprite.animation.timeScale = this.player.speed;
      this.land.SPEED = this.player.sprite.animation.timeScale*-1;
      // this.player.sprite.animations.getAnimation('WALK').delay = this.elapsed
    }
    if(this.game.time.now - this.startPress > 1000){
      this.player.animate("IDLE");
      this.land.SPEED = 0;
    }
	}
